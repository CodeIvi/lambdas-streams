    ...
    public static void main(String[] args) {
        ...
        while (!"exit".equalsIgnoreCase(input)
            ...
            if (instruccion.length != 3) {
                ...
                // ["+","23","45"]
                if (mapOperaciones.containsKey(instruccion[0])) {
                    try {
                        oper1 = Double.parseDouble(instruccion[1]);
                    } catch (NumberFormatException nfe) {
                        System.out.println("Formato de 1er parámetro " + instruccion[1] + " incorrecto");
                        continue;
                    }
                    try {
                        oper2 = Double.parseDouble(instruccion[2]);
                    } catch (NumberFormatException nfe) {
                        System.out.println("Formato de 2º parámetro " + instruccion[2] + " incorrecto");
                        continue;
                    }
                    // Ejecuta la operación y muestra el resultado
                    Operacion op = mapOperaciones.get(instruccion[0]);
                    Double result = op.oper(oper1, oper2);
